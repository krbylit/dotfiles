...
function M:render_table(job, opts)
  ...
  local row = function(key, value)
    ...
    rows[#rows + 1] = ui.Row({ ui.Line(key):style(styles.row_label), ui.Text(value):style(styles.row_value) }):height(h)
  end
  ...
  row(
    ...
    filesystem_error
      or (
        (filesystem_extra.avail_space and filesystem_extra.total_space and filesystem_extra.avail_space_percent)
...
function M:spot(job)
  ...
  ya.spot_table(
    job,
    self
      :render_table(job, { show_plugins_section = true })
