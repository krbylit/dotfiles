...
function M:peek(job)
  ...
  debug_log(
    "Preview for: " .. tostring(job.file.url) .. ", skip=" .. tostring(skip_val) .. ", area.h=" .. tostring(area_h)
  )
  ...
  if lines == "" then
    if skip > 0 then
      ...
      debug_log(
        "Empty output because skip ("
          .. skip
          .. ") > total lines ("
          .. actual_lines_in_file
          .. "). Resetting skip to "
          .. new_skip
      )
    ...
    if lines_read_for_display < limit and skip > 0 and i < skip + limit then
      ...
      debug_log(
        string.format(
          "Reached end of file at line %d while reading. Adjusting skip to %d.",
          actual_lines_in_file,
          new_skip
        )
      )
...
function M:seek(job)
  ...
  debug_log(
    string.format(
      "Calculated (from cx.skip): current_skip=%d, units=%d, new_skip=%d",
      current_skip,
      job.units,
      new_skip
    )
