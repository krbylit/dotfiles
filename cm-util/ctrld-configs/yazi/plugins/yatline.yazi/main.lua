...
function Yatline.string.get:hovered_name(config)
  ...
  if trimed then
    ...
    local trimmed_linked = #linked ~= 0
        and link_delimiter .. trim_filename(string.sub(linked, #link_delimiter + 1, -1), max_length, trim_length)
      or ""
...
return {
  setup = function(_, config, pre_theme)
    ...
    config.theme = (not rt.term.light and config.theme_dark) or (rt.term.light and config.theme_light) or config.theme
