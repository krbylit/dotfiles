function gtbeads --description "Advance a crew member from plan to beads workflow"
    set -l force 0
    set -l args $argv
    ...
    if test (count $args) -lt 1
        echo "Usage: gtbeads [--force] <crew> <worker>" >&2
        echo "   or: gtbeads [--force] <crew/worker>" >&2
        return 1
    end
    ...
    if test "$stage" != plan; and test $force -ne 1
        echo "Error: $crew/$worker is at stage '$stage', expected 'plan' before running beads-workflow" >&2
        echo "Use 'gtstatus $crew $worker' to inspect it or rerun with --force to bypass the guard." >&2
        return 1
    end

    gt sling beads-workflow "$crew/$worker" \
        --var feature="$feature"
    or return $status

    _gt_work write-current $crew $worker $feature beads
    or return $status
    _gt_work append-history $crew $worker beads $feature
    or return $status

    echo "Advanced $crew/$worker to beads-workflow"
