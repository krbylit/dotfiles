...
_kMetaTable.__pairs = function(obj)
  return pairs(_kMetaTable._k[obj])
end
_kMetaTable.__len = function(obj)
  return #_kMetaTable._k[obj]
end
_kMetaTable.__tostring = function(obj)
  ...
  if _kMetaTable._k[obj] then
    local width = 0
    for k, v in pairs(_kMetaTable._k[obj]) do
      width = width < #tostring(k) and #tostring(k) or width
    end
...
elementSearchHamster = function(element, searchParameters, isPattern, includeParents, seen)
  ...
  if getmetatable(element) == objectMT then
    if fnutils.contains(seen, element) then
      return results
    end
...
objectMT.__index = function(self, _)
  if type(_) == "string" then
    ...
    if not matchName then
      matchName = _:match("^do(.+)$")
    end
    if not matchName then
      matchName = _:match("^(.+)WithParameter$")
    end
    if not matchName then
      matchName = _
    end
    local formalName = matchName:match("^AX[%w%d_]+$") and matchName
      or "AX" .. matchName:sub(1, 1):upper() .. matchName:sub(2)
      ...
      for i, v in ipairs(objectMT.attributeNames(self) or {}) do
        if v == formalName and objectMT.isAttributeSettable(self, formalName) then
          return function(self, ...)
            return objectMT.setAttributeValue(self, formalName, ...)
          end
        end
      end
    ...
    elseif _:match("^do") then
      ...
      for i, v in ipairs(objectMT.actionNames(self) or {}) do
        if v == formalName then
          return function(self, ...)
            return objectMT.performAction(self, formalName, ...)
          end
        end
      end
      ...
      for i, v in ipairs(objectMT.attributeNames(self) or {}) do
        if v == formalName then
          return function(self, ...)
            return objectMT.attributeValue(self, formalName, ...)
          end
        end
      end
      ...
      for i, v in ipairs(objectMT.parameterizedAttributeNames(self) or {}) do
        if v == formalName then
          return function(self, ...)
            return objectMT.parameterizedAttributeValue(self, formalName, ...)
          end
        end
      end
    end
...
objectMT.matches = function(self, searchParameters, isPattern)
  ...
  if type(searchParameters) == "string" or #searchParameters > 0 then
    searchParameters = { role = searchParameters }
  end
  local answer = nil
          ...
          if type(v2) == type(result) then
            if type(v2) == "string" then
              partialAnswer = partialAnswer
                or (not isPattern and result == v2)
                or (isPattern and (type(result) == "string") and result:match(v2))
...
buildTreeHamster = function(self, depth, withParents, seen)
  ...
  seen = seen or {}
  if getmetatable(self) == objectMT then
    local seenBefore = fnutils.find(seen, function(_)
      return _._element == self
    end)
    if seenBefore then
      return seenBefore
    end
      ...
      if k ~= "_element" then
        ...
        elseif getmetatable(v) == objectMT then
          if not withParents and fnutils.contains(parentLabels, k) then
            ...
            thisObject[k] = fnutils.find(seen, function(_)
              return _._element == v
            end) or v
          else
  ...
  while #searchables > 0 and not found do
    ...
    if not fnutils.contains(seen, element) then
      ...
      if element:matchesCriteria(levelCriteria, levelCriteria._pattern) then
        ...
        if count == 0 then
          found = element
        end -- allow for "selecting the 2nd found", useful for arrays
      end
...
local searchPathWrapper = function(self, criteria, depth, withParents, path, criteriaSeen)
  ...
  if not criteriaSeen then
    ...
    if criteria[1] and type(criteria[1]._includeSelf) == "nil" then
      criteria[1]._includeSelf = true
    end
  ...
  while not failed and position <= #criteria do
    ...
    if step then
      if not fnutils.contains(criteriaSeen[levelCriteria], step) then
        for i, v in ipairs(thingsToIgnore) do
          table.insert(criteriaSeen[levelCriteria], v)
        end
...
objectMT.next = function(self)
  ...
  if uservalue then
    ...
    return searchPathWrapper(
      self,
      uservalue.criteria,
      uservalue.depth,
      uservalue.withParents,
      uservalue.path,
      uservalue.criteriaSeen
    )
  else
