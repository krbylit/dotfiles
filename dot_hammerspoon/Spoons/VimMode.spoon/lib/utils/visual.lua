...
visualUtils.getNewRange = function(currentRange, motionRange, caretPosition)
  ...
  if currentRange.finish == motionRange.finish or currentRange.start == motionRange.finish then
  ...
  if noSelection then
    return {
      caretPosition = (motionDirection == "left" and motionRange.start) or motionRange.finish,
      range = motionRange,
    }
  end
  ...
  local newValue = (motionDirection == "left" and motionRange.start) or motionRange.finish
